...
static PyObject *grid_subsampling_compute(PyObject *self, PyObject *args, PyObject *keywds)
{
	...
	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|$OOOsi", kwlist, &points_obj, &features_obj, &classes_obj, &sampleDl_obj, &method_buffer, &verbose))
	{
	...
	if (classes_obj == NULL)
		use_classes = false;
	bool use_sampleDl_arr = !PyFloat_Check(sampleDl_obj);
	...
	if (use_sampleDl_arr)
		sampleDl_array = PyArray_FROM_OTF(sampleDl_obj, NPY_FLOAT, NPY_IN_ARRAY);
	...
	if (use_sampleDl_arr && sampleDl_array == NULL)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
		Py_XDECREF(sampleDl_array);
		PyErr_SetString(PyExc_RuntimeError, "Error converting input sampleDl to numpy arrays of type float32");
		return NULL;
	}
	...
	if (use_sampleDl_arr && ((int)PyArray_NDIM(sampleDl_array) != 1 || (int)PyArray_DIM(sampleDl_array, 0) != 3))
	{
		Py_XDECREF(points_array);
		Py_XDECREF(classes_array);
		Py_XDECREF(features_array);
		PyErr_SetString(PyExc_RuntimeError, "Wrong dimensions : sampleDl.shape is not (3,)");
		return NULL;
	}
	...
	if (use_sampleDl_arr) {
		float* i = (float*)PyArray_DATA(sampleDl_array);
		sampleDl = PointXYZ(i[0], i[1], i[2]);
	}
	else {
		float i = (float)PyFloat_AsDouble(sampleDl_obj);
		sampleDl = PointXYZ{i, i, i};
	}
