...
void grid_subsampling(vector<PointXYZ>& original_points,
	...
	size_t sampleNX = (size_t)floor((maxCorner.x - originCorner.x) / sampleDl.x) + 1;
	size_t sampleNY = (size_t)floor((maxCorner.y - originCorner.y) / sampleDl.y) + 1;
	//size_t sampleNZ = (size_t)floor((maxCorner.z - originCorner.z) / sampleDl.z) + 1;

	if (verbose > 0){
		size_t sampleNZ = (size_t)floor((maxCorner.z - originCorner.z) / sampleDl.z) + 1;
		std::cout << "sampleDl = " << sampleDl << std::endl;
		std::cout << "min/max/originCorner = " << minCorner << " / " << maxCorner << " / " << originCorner << std::endl;
		std::cout << "sampleNX/Y/Z = " << sampleNX << " / " << sampleNY << " / " << sampleNZ << std::endl;
	}
	...
	for (auto& p : original_points)
	{
		...
		iX = (size_t)floor((p.x - originCorner.x) / sampleDl.x);
		iY = (size_t)floor((p.y - originCorner.y) / sampleDl.y);
		iZ = (size_t)floor((p.z - originCorner.z) / sampleDl.z);
